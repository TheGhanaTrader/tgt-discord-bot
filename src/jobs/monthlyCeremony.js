...
async function runMonthlyCeremonyCore({ client, monthKeyOverride, reason, dryRun }) {
  ...
  for (const t of top10Sales) {
    ...
    const cert = await generateCertificatePNG({
      ...
      userId: t.userId,
      rank: rankLabel,
      reward: String(entry?.reward || entry?.prize || entry?.prizeName || reward || "Top 10 Recognition"),
      month: monthKey,
    ...
    await recordCertificate({
      ...
      category,
      rankLabel,
      rewardLabel: String(entry?.reward || entry?.prize || entry?.prizeName || reward || "Top 10 Recognition"),
      code: finalCode,
      filePath,
